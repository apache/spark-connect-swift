  ...
  func defineOutput() async throws {
    ...
    if response.sparkVersion.version.starts(with: "4.1") {
      ...
      try await #require(throws: SparkConnectError.OutputTypeUnspecified) {
        try await client.defineOutput(dataflowGraphID, "ds1", "unspecified")
      }
      #expect(try await client.defineOutput(dataflowGraphID, "ds2", "materializedView"))
      #expect(try await client.defineOutput(dataflowGraphID, "ds3", "table"))
      #expect(try await client.defineOutput(dataflowGraphID, "ds4", "temporaryView"))
    }
    await client.stop()
  }
