...
public struct Column: Sendable {
  ...
  /// If `otherwise(_:)` is not defined at the end, null is returned for
  /// unmatched conditions. This can only be applied on a ``Column`` previously generated
  /// by `when(_:_:)` before `otherwise` is applied.
  ///
  /// ```swift
  /// df.select(when(col("age") > 60, "senior").when(col("age") > 21, "adult").otherwise("minor"))
  /// ```
  /// - Parameters:
  ///   - condition: A condition ``Column``.
  ///   - value: A value ``Column`` to return when the condition is true.
  /// - Returns: A ``Column``.
  public func when(_ condition: Column, _ value: Column) throws -> Column {
    guard isChainableCaseWhen else {
      throw SparkConnectError.InvalidArgument
    }
    var function = self.expr.unresolvedFunction
    function.arguments += [condition.expr, value.expr]
    var expr = Spark_Connect_Expression()
  ...
  /// - Returns: A ``Column``.
  public func when(_ condition: Column, _ value: some SparkLiteral) throws -> Column {
    return try when(condition, value.toLiteralColumn)
  }

  /// Returns the given value if all conditions of the preceding `when(_:_:)` are unmatched.
  /// This can only be applied once on a ``Column`` previously generated by `when(_:_:)`.
  /// - Parameter value: A value ``Column``.
  /// - Returns: A ``Column``.
  public func otherwise(_ value: Column) throws -> Column {
    guard isChainableCaseWhen else {
      throw SparkConnectError.InvalidArgument
    }
    var function = self.expr.unresolvedFunction
    function.arguments.append(value.expr)
    var expr = Spark_Connect_Expression()
    expr.unresolvedFunction = function
    return Column(expr)
  }
  ...
  /// - Returns: A ``Column``.
  public func otherwise(_ value: some SparkLiteral) throws -> Column {
  ...
  private var isChainableCaseWhen: Bool {
    return expr.unresolvedFunction.functionName == "when"
      && expr.unresolvedFunction.arguments.count % 2 == 0
  }
